...
def load_csv_docs(data_dir_path: str) -> list[Document]:
    ...
    for path in csv_path:
        loader = CSVLoader(file_path=path, encoding="utf-8") #UTF-8に追加変更
...
def create_vector_search_index(qdrant_client: QdrantClient, index_name: str) -> None:
    ...
    # すでにコレクションが存在するか確認 (追加変更分）
    if qdrant_client.collection_exists(index_name):
...
def add_documents_to_es(
    ...
    #helpers.bulk(es, insert_docs, request_timeout=300)
    try:
        helpers.bulk(es, insert_docs, chunk_size=10, request_timeout=300)
